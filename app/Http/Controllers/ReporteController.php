...
class ReporteController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        if ($user->rol == 'residente') {
            $query = Reporte::with('usuario')->latest();

            // Aplicar filtros si vienen por parámetro
    ...
    // Dashboard de residentes con filtros
...
public function store(Request $request)
{
    ...
    $this->notificarNuevoReporte($reporte);

    return redirect('/dashboard')->with('success', 'Reporte creado exitosamente');
}
    // Ver detalle
 ...
 public function actualizar(Request $request, $id)
{
    ...
    $this->notificarReporteActualizado($reporte, $request->estado, auth()->id());

    // NOTIFICACIÓN ANTIGUA (mantener compatibilidad)
...
public function apoyar($id)
{
    ...
    // 🔔 ENVIAR NOTIFICACIÓN AL CREADOR DEL REPORTE
    $this->notificarNuevoApoyo($reporte, auth()->id());

    return back()->with('success', 'Apoyo registrado');
}

    // MÉTODOS DE NOTIFICACIÓN
    private function notificarNuevoReporte($reporte)
    {
        // Obtener todos los administradores y super administradores
        $admins = Usuario::whereIn('rol', ['admin', 'superadmin'])->get();
        
        foreach ($admins as $admin) {
            NotificacionTiempoReal::crearNotificacion(
                $admin->id,
                '🚨 Nuevo Reporte Creado',
                "Un nuevo reporte requiere tu atención. ID: #{$reporte->id} - {$reporte->categoria}",
                'urgent',
                'reporte',
                ['reporte_id' => $reporte->id, 'categoria' => $reporte->categoria]
            );
        }
    }

    private function notificarNuevoApoyo($reporte, $apoyoUserId)
    {
        // Notificar al creador del reporte
        NotificacionTiempoReal::crearNotificacion(
            $reporte->usuario_id,
            '👍 ¡Tu reporte tiene un nuevo apoyo!',
            "Alguien más apoya tu reporte #{$reporte->id}",
            'success',
            'apoyo',
            ['reporte_id' => $reporte->id, 'apoyo_user_id' => $apoyoUserId]
        );
    }

    private function notificarReporteActualizado($reporte, $nuevoEstado, $adminId)
    {
        // Notificar al creador del reporte sobre cambios
        NotificacionTiempoReal::crearNotificacion(
            $reporte->usuario_id,
            '📋 Estado de Reporte Actualizado',
            "Tu reporte #{$reporte->id} ahora está: {$this->formatEstado($nuevoEstado)}",
            'info',
            'reporte',
            ['reporte_id' => $reporte->id, 'nuevo_estado' => $nuevoEstado, 'admin_id' => $adminId]
        );
    }
